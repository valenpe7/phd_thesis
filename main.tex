...
Let us now consider ideal relativistic flying parabolic mirror (i.e., the reflection coefficient equal to unity, no wavefront aberration). The distribution of electromagnetic wave packet reflected from the mirror can be again obtained by finding the distribution in the moving frame and then performing the transformation back to the stationary frame. As shown in [], the nominal focal length of the parabolic mirror in the moving frame is $ \gamma $ times shorter than in the stationary frame. This alters the situation to the $ 4 \pi $-spherical focusing scheme.

in spherical coordinates $ \rho = \sqrt{x^2 + y^2 + z^2} $ (radial coordinate), $ \theta = \arccos \left( z / \sqrt{x^2 + y^2 + z^2} \right)$ (polar coordinate), and $ \phi = \arctan \left( y / x \right)$ (azimuthal coordinate).

for the radially polarized beam (field distribution of the azimuthally polarized beam at focus can be obtained by exchanging the expressions for the electric and magnetic fields of the radially polarized beam).

The general mathematical expressions for the spatiotemporal distribution of electric and magnetic fields of the radially polarized beam in the focal region of mirror in the stationary frame of reference is given by
...
\end{eqnarray}
\end{subequations}
Here $ w_e $ is the effective beam radius ($ w_e = w_0 \sqrt{2p + m + 1}$ for the $ p $-th radial, $ m $-th azimuthal Laguerre-Gaussian beam), $ \lambda_0 = 2 \pi c / \omega_0 $ is the beam wavelength, $ j_{n} \left( \cdot \right) $ is the $ n $-th order spherical Bessel function of the first kind, 
\begin{subequations}
\begin{eqnarray}
\begin{split}
