...
Langmuir waves are perhaps the most essential physical entities considered within this dissertation thesis. They play a key role in the concepts of laser-wakefield acceleration and relativistic flying mirrors, which are presented in the following sections (Secs.~ and ). Here we describe the propagation of finite-amplitude (i.e., the frequency of the wave depends on the amplitude) Langmuir and electromagnetic waves in cold collisionless plasma. The exact solution to this problem was first found in Ref.~\citenum{Akhiezer1956}.

Starting with the Maxwell's equations and the hydrodynamic equations for the electron and ion fluid components, we may obtain the system of equations for coupled Langmuir (electrostatic) and electromagnetic waves in the following compact form \cite{Farina2001, Bulanov2013, Bulanov2021},
\begin{subequations}\label{eq:coupled_em_and_langmuir_wave}
\begin{equation}\label{eq:eq_for_vector_potential}
\laplace{\vec{a}} - \diffp[2]{\vec{a}}{t} - \diffp{}{t} \grad{\varphi} = \vec{\upbeta}_e n_e - \vec{\upbeta}_i n_i,
\end{equation}
\begin{equation}\label{eq:eq_for_scalar_potential}
\laplace{\varphi} = n_e - n_i,
\end{equation}
\begin{equation}\label{eq:eq_for_density}
\diffp{n_{\alpha}}{t} + n_{\alpha} \grad{\vec{\upbeta}_{\alpha}} = 0,
\end{equation}
...
\end{equation}
\end{subequations}
Here, subscripts $ e $ and $ i $ refer to the electron and ion fluid components, subscript $ \alpha $ is either $ e $ or $ i $. Here and below we use dimensionless variables; quantities having the units of length and time are normalized by $ c / \omega_{pe} $ and $ \omega_{pe} $, respectively, velocities are normalized by $ c $, momenta by $ m_{\alpha} c $, scalar and vector potentials are both normalized by $ m_e c^2 / e $, and the number densities are normalized by $ n_0 $, where $ n_0 $ is the electron density at the equilibrium state.

In one-dimensional geometry (i.e., $ \partial / \partial x = \partial / \partial y = 0 $) the dependence of all quantities in Eqs.~(\ref{eq:coupled_em_and_langmuir_wave}) on the coordinate and time can be expressed via new variables $ \zeta = z - \beta_g t $ and $ \tau = t - \beta_g z $, where $ \beta_g $ is the group velocity of the electromagnetic wave (i.e., we search for the solution in the form of a plane wave traveling at constant velocity). For the sake of simplicity, we further assume that the electromagnetic wave is circularly polarized; the equations for the scalar, $ \varphi = \varphi \left( \zeta \right) $, and vector, $ a_x + i a_y = a \left( \zeta \right) \exp \left( i \omega \tau \right) $, potentials then read as \cite{Farina2001, Bulanov2013, Bulanov2021}
\begin{equation}\label{eq:eq_for_scalar_potential_2}
\diff[2]{\varphi}{\zeta} = \frac{\beta_g}{1 - \beta_g^2} \left(\frac{\psi_e}{R_e} - \frac{\psi_i}{R_i} \right),
\end{equation}
\begin{equation}\label{eq:eq_for_vector_potential_2}
\diff[2]{a}{\zeta} + \omega^2 a = a \frac{\beta_g}{1 - \beta_g^2}  \left(\frac{\rho_e}{R_e} - \frac{\rho_i}{R_i} \right),
\end{equation}
where $ \omega $ is the angular frequency of the electromagnetic wave, $ \psi_{\alpha} = \mathit{\Gamma}_{\alpha} + \rho_{\alpha} \varphi $, and $ R_{\alpha} = \sqrt{\psi_{\alpha}^2 - \left(1 - \beta_g^2 \right) \left(1 + \rho_{\alpha}^2 a^2 \right)} $. The value of $ \mathit{\Gamma}_{\alpha} $ is determined by the boundary conditions; e.g., if the electromagnetic field amplitude is constant (i.e., $ a = a_0 $ and $ \varphi = 0 $) and the plasma is at rest (i.e., $ p_{\alpha} = 0 $) at infinity, then $ \mathit{\Gamma}_{\alpha} = \sqrt{1 + \rho_{\alpha}^2 a_0^2} $. The Lorentz factor and the number density of the electron and ion components can be expressed as functions of the potentials \cite{Farina2001, Bulanov2013, Bulanov2021},
...
In what follows we neglect the motion of ions, i.e., $ \rho_i = 0 $ (note that the ion motion affects the value of scalar potential in the Langmuir wave \cite{Bulanov2021}). If we set the electromagnetic wave amplitude to zero, i.e., $ a = a_0 = 0 $, Eq.~(\ref{eq:eq_for_scalar_potential_2}) describes free Langmuir waves,
...
Let us now consider the Langmuir wave generated by given non-evolving electromagnetic pulse. In this case, Eq.~(\ref{eq:eq_for_scalar_potential_2}) takes the following form \cite{Bulanov2016, Bulanov2021},
\begin{equation}\label{eq:eq_for_scalar_potential_4}
\diff[2]{\varphi}{\zeta} = \frac{\beta_g}{1 - \beta_g^2} \left( \frac{1 + \varphi}{\sqrt{\left( 1 + \varphi \right)^2 - \left( 1 - \beta_g^2 \right) \left( 1 + a^2 \left( \zeta \right) \right)}} - \frac{1}{\beta_g} \right).
\end{equation}
In the limit $ \beta_g = \beta_{ph} \rightarrow 1 $ (corresponding to the case of low-density plasma, when the front of the electromagnetic pulse propagates at velocity close to $ c $), Eq.~(\ref{eq:eq_for_scalar_potential_4}) simplifies to \cite{Bulanov1989, Berezhiani1990, Esarey2009, Bulanov2016, Bulanov2021}
...
\end{equation}
Analytical solution to Eq.~(\ref{eq:eq_for_scalar_potential_5}) can be found in the implicit form for square laser pulse profiles, e.g., $ a = a_0 $ for $ \zeta \leq 0 $ and $ a = 0 $ for $ \zeta > 0 $, as \cite{Bulanov1989, Berezhiani1990, Bulanov2016, Bulanov2021}
\begin{equation}\label{key}
...
The Coulomb gauge is assumed (i.e., $ \div{\vec{a}} = 0 $).
...
\section{Relativistic mirrors in underdense plasmas}
%\input{dat/1-0.tex}
