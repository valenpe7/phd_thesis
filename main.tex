...
Let us consider ideally reflecting mirror (i.e., the reflection coefficient of the mirror is equal to unity) moving along the $ +z $-axis at a constant velocity $ v = \beta c $. The electric field of the counter-propagating (i.e., moving along the $ -z $-axis) electromagnetic wave packet linearly polarized along the $ x $-axis is expressed in the stationary frame of reference, $ S_1 $, as follows
\begin{equation}\label{eq:incident_lab_frame_1}
\vec{E}_i \left(x, y, z; t \right) = \vec{\hat{x}} \, E_0 \left(x, y \right) \exp \left[ - \frac{1}{\tau^2} \left(t + \frac{z}{c}\right)^2 \right] \exp \left[ i \omega \left(t + \frac{z}{c}\right) \right],
\end{equation}
where $ E_0 \left(x, y \right) $ is the peak amplitude of the electric field, $ \tau $ is the wave packet duration (note that the full-width-at-half-maximum duration can be obtained as $ \tau_{\mathrm{FWHM}} = 2 \sqrt{\ln 2} \tau $), and $ \omega $ is the center angular frequency of the wave packet. 
...
\vec{E}^{\prime}_{i, \parallel} = \vec{E}_{i, \parallel}, \quad \vec{E}^{\prime}_{i, \bot} = \frac{\vec{E}_{i, \bot} + \beta c \, \vec{\hat{z}} \times \vec{B}_{i}}{\sqrt{1 - \beta^2}},
\end{equation}
where subscripts $ \parallel $ and $ \bot $ denote the field polarization components parallel and perpendicular to the direction of mirror motion and $ \vec{B}_{i} = \vec{\hat{z}} / c \times \vec{E}_i $ is the magnetic field of the incident wave packet in $ S_1 $. Note that the origin of $ M $ is located at $ \left( 0, 0, z_0 \right) $. In $ M $, the electric field of the incident wave packet, Eq.~(\ref{eq:incident_lab_frame_1}), takes the following form,
\begin{equation}\label{eq:incident_boost_frame}
\vec{E}^{\prime}_i \left(x^{\prime}, y^{\prime}, z^{\prime}; t^{\prime} \right) = \vec{\hat{x}}^{\prime} \, E^{\prime}_0 \left(x^{\prime}, y^{\prime} \right) \exp \left[ - \frac{1}{\tau^{\prime 2}} \left(t^{\prime} + \frac{z^{\prime}}{c}\right)^2 \right] \exp \left[ i \omega^{\prime} \left(t^{\prime} + \frac{z^{\prime}}{c}\right) \right],
\end{equation}
where
\begin{equation}\label{eq:coeff_boost}
E^{\prime}_0 \left(x^{\prime}, y^{\prime} \right) = \sqrt{\frac{1 + \beta}{1 - \beta}} E_0 \left(x, y \right), \quad \tau^{\prime} = \sqrt{\frac{1 - \beta}{1 + \beta}} \tau, \quad \omega^{\prime} = \sqrt{\frac{1 + \beta}{1 - \beta}} \omega.
\end{equation}
...
\vec{E}^{\prime}_r \left(x^{\prime}, y^{\prime}, z^{\prime}; t^{\prime} \right) = \vec{\hat{x}}^{\prime} \, E^{\prime}_0 \left(x^{\prime}, y^{\prime} \right) \exp \left[ - \frac{1}{{\tau^{\prime 2}}} \left(t^{\prime} - \frac{z^{\prime}}{c}\right)^2 \right] \exp \left[ i \omega^{\prime} \left(t^{\prime} - \frac{z^{\prime}}{c}\right) \right].
\end{equation}
...
\begin{equation}\label{eq:inverse_lorentz_transform_fields}
	\vec{E}^{\prime \prime}_{r, \parallel} = \vec{E}^{\prime}_{r, \parallel}, \quad \vec{E}^{\prime \prime}_{r, \bot} = \frac{\vec{E}^{\prime}_{r, \bot} - \beta c \, \vec{\hat{z}} \times \vec{B}^{\prime}_{r}}{\sqrt{1 - \beta^2}},
\end{equation}
where $ \vec{B}^{\prime}_{r} = -\vec{\hat{z}} / c \times \vec{E}^{\prime}_r $ is the magnetic field of the reflected wave packet in $ M $. In $ S_2 $, the electric field of the reflected wave packet reads as
\begin{equation}\label{eq:reflected_lab_frame_1}
\vec{E}^{\prime \prime}_r \left(x^{\prime \prime}, y^{\prime \prime}, z^{\prime \prime}; t^{\prime \prime} \right) = \vec{\hat{x}}^{\prime \prime} \, E^{\prime \prime}_0 \left(x^{\prime \prime}, y^{\prime \prime} \right) \exp \left[ - \frac{1}{\tau^{\prime \prime 2}} \left(t^{\prime \prime} - \frac{z^{\prime \prime}}{c}\right)^2 \right] \exp \left[ i \omega^{\prime \prime} \left(t^{\prime \prime} - \frac{z^{\prime \prime}}{c}\right) \right],
\end{equation}
where
\begin{equation}\label{eq:coeff_lab}
E^{\prime \prime}_0 \left(x^{\prime \prime}, y^{\prime \prime} \right) = \sqrt{\frac{1 + \beta}{1 - \beta}} E^{\prime}_0 \left(x^{\prime}, y^{\prime} \right), \quad \tau^{\prime \prime} = \sqrt{\frac{1 - \beta}{1 + \beta}} \tau^{\prime}, \quad \omega^{\prime \prime} = \sqrt{\frac{1 + \beta}{1 - \beta}} \omega^{\prime}.
\end{equation}
...
\begin{split}
\vec{E}^{\prime \prime}_r \left(x, y, z; t \right) = & \ \vec{\hat{x}} \, \frac{1 + \beta}{1 - \beta} E_0 \left(x, y \right) \exp \left[ - \left(\frac{1 + \beta}{1 - \beta}\right)^2 \frac{1}{\tau^2} \left(t - \frac{z - z_0}{c}\right)^2 \right] \\ 
& \times \exp \left[ i \frac{1 + \beta}{1 - \beta} \omega \left(t - \frac{z - z_0}{c}\right) \right].
\end{split}
\end{equation}
For a Gaussian beam of width $ w_0 $, $ E_0 \left( x, y \right) = E_0 \exp \left[-\left( x^2 + y^2 \right) / w_0^2 \right] $, and thus the total energy and peak intensity of the reflected pulse is given by 
\begin{equation}\label{eq:energy_and_intensity}
\mathcal{E}^{\prime \prime}_r = \left(\frac{1 + \beta}{1 - \beta}\right) \sqrt{\frac{\pi^3}{2}} \epsilon_0 c \tau w_0^2 E_0^2 , \quad I^{\prime \prime}_r = \left(\frac{1 + \beta}{1 - \beta}\right)^2 \frac{\epsilon_0 c}{2} E_0^2.
\end{equation}
...
The above derivation shows that the reflected wave packet possesses several striking features: its electric field is amplified by the factor $ \approx 4 \gamma^2 $, its duration is compressed by $ \approx 4 \gamma^2 $, and its angular frequency is upshifted by $ \approx 4 \gamma^2 $. Furthermore, its energy and intensity are enhanced by $ \approx 4 \gamma^2 $ and $ \approx 16 \gamma^4 $, respectively. Assuming a laser pulse with parameters corresponding to a typical $ \mathrm{PW} $-class titanium-doped sapphire system, i.e., $ 0.8 \ \mathrm{\mu m} $ wavelength, $ 30 \ \mathrm{fs} $ duration, and $ 10^{20} \ \mathrm{W / cm^2} $ peak intensity, being reflected from ideal relativistic flying flat mirror with $ \gamma = 10 $, one obtains a pulse with wavelength of $ 2 \ \mathrm{nm} $, duration of $ 75 \ \mathrm{as} $, and the peak intensity of $ 1.6 \times 10^{25} \ \mathrm{W / cm^2} $.

\subsection{Relativistic flying parabolic mirror}

The general mathematical expressions for the spatiotemporal field distribution of the RLF with the radial polarization are obtained as,

\begin{equation}\label{key}
\begin{split}
\vec{E}^{\prime \prime}_{r} = & \ \gamma \frac{1 + \beta}{1 - \beta} \sqrt{\frac{3 \pi}{\epsilon_0 c}} \frac{\pi \omega w_e \sqrt{I_0}}{4c} \\[2mm]
& \times
\begin{bmatrix}
	\left\lbrace -j_0 \left( \omega^{\prime} R / c \right) \sin \left(\omega^{\prime} T \right) \Upsilon_1 + \beta j_1 \left( \omega^{\prime} R / c \right) \cos \left(\omega^{\prime} T \right) \Upsilon_2 \right\rbrace \cos \phi \\[1mm]
	\left\lbrace -j_0 \left( \omega^{\prime} R / c \right) \sin \left(\omega^{\prime} T \right) \Upsilon_1 + \beta j_1 \left( \omega^{\prime} R / c \right) \cos \left(\omega^{\prime} T \right) \Upsilon_2 \right\rbrace \sin \phi \\[1mm]
	\left( 1 / \gamma \right) j_0 \left( \omega^{\prime} R / c \right) \sin \left(\omega^{\prime} T \right) \Upsilon_2
\end{bmatrix}
\end{split}
\end{equation}
...
\begin{equation}\label{key}
	\begin{split}
		\vec{B}^{\prime \prime}_{r} = & \ \frac{\gamma}{c} \frac{1 + \beta}{1 - \beta} \sqrt{\frac{3 \pi}{\epsilon_0 c}} \frac{\pi \omega w_e \sqrt{I_0}}{4c} \\[2mm]
		& \times
		\begin{bmatrix}
			\left\lbrace -j_1 \left( \omega^{\prime} R / c \right) \cos \left(\omega^{\prime} T \right) \Upsilon_2 + \beta j_0 \left( \omega^{\prime} R / c \right) \sin \left(\omega^{\prime} T \right) \Upsilon_1 \right\rbrace \sin \phi \\[1mm]
			\left\lbrace j_1 \left( \omega^{\prime} R / c \right) \cos \left(\omega^{\prime} T \right) \Upsilon_2 - \beta j_0 \left( \omega^{\prime} R / c \right) \sin \left(\omega^{\prime} T \right) \Upsilon_1 \right\rbrace \cos \phi \\[1mm]
			0
		\end{bmatrix}
	\end{split}
\end{equation}
...
\begin{equation}\label{key}
\begin{split}
\Upsilon_1 = & \ \frac{1}{2} \left\lbrace \frac{\cos \theta + \beta}{1 + \beta \cos \theta} \exp \left[- \frac{\Delta \omega^{\prime 2}}{4} \left( T + \frac{R}{c} \right)^2 \right] \right. \\
...
\begin{equation}\label{key}
\begin{split}
\Upsilon_2 = & \ \frac{1}{2} \left\lbrace \frac{\sin \theta}{\gamma \left( 1 - \beta \cos \theta \right) } \exp \left[- \frac{\Delta \omega^{\prime 2}}{4} \left( T - \frac{R}{c} \right)^2 \right] \right. \\
