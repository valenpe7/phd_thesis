...
Starting with the Maxwell's equations and the hydrodynamic equations for the electron and ion fluid components, we may obtain the system of equations for coupled Langmuir (electrostatic) and electromagnetic waves in the following compact form \cite{Farina2001, Bulanov2013, Bulanov2020},
...
If we set the electromagnetic wave amplitude to zero (i.e., a $\left( \zeta \right) = a_0 = 0 $), Eq.~(\ref{eq:eq_for_scalar_potential_2}) describes free Langmuir waves. If we further neglect the effect of ions, Eq.~(\ref{eq:eq_for_scalar_potential_2}) gives us the dependence of the electron Lorentz factor, $ \gamma_e $, on the coordinate, $ \zeta $, (note that the coordinate is for the Langmuir wave redefined as $ \zeta = z - \beta_{ph} t $, where $ \beta_{ph} $ is the phase velocity of the Langmuir wave) in the implicit form as
\begin{equation}\label{key}
\frac{1}{\sqrt{2}} \zeta = \beta_{ph} \left[ \sqrt{\gamma_m + 1} \, E \left( \mathit{\Psi} \rvert \kappa \right) + \frac{1}{\sqrt{\gamma_m + 1}} \, F \left( \mathit{\Psi} \rvert \kappa \right) \right] - \sqrt{\gamma_m - \gamma_e},
\end{equation}
where 
\begin{equation}\label{key}
...
\end{equation}
are the incomplete elliptic integrals of the first and second kinds \cite{Gradstein}, respectively, $ \mathit{\Psi} = \arcsin \sqrt{\left( \gamma_m - \gamma_e \right) / \left( \gamma_m - 1 \right)} $, $ \kappa = \left( \gamma_m - 1 \right) / \left( \gamma_m + 1 \right) $, and $ \gamma_m $ is the maximum value of $ \gamma_e $ in the Langmuir wave.
