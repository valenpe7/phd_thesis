...
\begin{figure}[t]
	...
	\begin{tikzpicture}[node distance = 2cm, auto]
		...
		\node [block, right of=uprostred, node distance=4cm] (vpravo) {Particle weighting\\[2mm] $ \left(\vec{x}, \vec{v}\right)_{p} \rightarrow \left(\rho, \vec{J}\,\right)_{i, j, k} $ };
	...
	\end{tikzpicture}
	\caption[]{\label{fig:pic_cycle} Computational cycle of the particle-in-cell method.}
\end{figure}

The distribution function $ f_{s} \left(\vec{r}, \vec{v}, t\right) $ of particle species $ s $ is obtained by summing the distribution functions of each quasi-particle $ f_{p, s} \left(\vec{r}, \vec{v}, t\right) $,
\begin{equation}
	\label{3.1.1}
	f_{s} =  \sum_{p} f_{p, s}.
\end{equation}
Here, we use the subscript $ p $ to denote quantities attributed to quasi-particles. The quasi-particle distribution function can be defined as 
\begin{equation}
	\label{3.1.2}
...
\end{equation}
where $ S_{r} \left(\vec{r} - \vec{r}_{p} \left(t\right) \right) $ and $ S_{v} \left(\vec{v} - \vec{v}_{p} \left(t\right) \right) $ are the shape functions for the spatial and velocity coordinates, respectively, and $ w_{p} $ is the weight of the quasi-particle (i.e., a number of physical particles which the quasi-particle represents).

The shape functions cannot be chosen arbitrarily, they have to satisfy the following requirements: (i) the shape function is even, (ii) the integral of the shape function is equal to unity, and (iii) the support of the shape function is compact. While these restrictive conditions still offer a wide range of options, the standard PIC method is essentially determined by the choice of the shape function in the velocity coordinate as a Dirac delta function and in the spatial coordinate as a m-th order b-spline basis function $ b_{m} $. Note that the choice of the shape functions has strong impact on the stability and accuracy of the simulation \cite{birdsall}. Higher-order basis functions result in less numerical noise and reduce non-physical phenomena in simulations, obviously at the cost of increased computational time.
