...
\end{equation}
where we assume that $ \gamma_e = \sqrt{1 + p_e^2 + a^2} $ (see also Sec.~\ref{label}). One can immediately see that Eq.~(\ref{eq:eq_for_long_momentum}) becomes singular when the denominator on the right-hand side vanishes, i.e., when the electron velocity, $ p_e / \gamma_e $, tends to the phase velocity of the Langmuir wave, $ \beta_{w} $. This singularity, which is also known as the gradient or cusp catastrophe, corresponds to the breaking of Langmuir wave.

The structure of the breaking Langmuir wave can be 

by solving Eq.~(\ref{eq:eq_for_density}) we can obtain the distribution of the electron density as
\begin{equation}\label{key}
n_e \left( \delta \zeta \right) = \frac{\beta_w}{\beta_w - \beta_e}
\end{equation}
...
In each case the singularity corresponds to the caustic in the plasma flow with inhomogeneous velocity.
...
%The right-hand side of Eq. 50 becomes singular
%when the denominator, mevph−p, tends to zero, i.e., when
%the electron velocity v becomes equal to the phase velocity
%of the wake wave, vph.



denominator in the right-hand side vanishes.

The singularity corresponds to the case where the electron velocity becomes equal to the wave phase velocity, which means wave breaking.

Here we analyze the solutions of this equation in the vicinity of the singularity.

The Langmuir wave break occurs if the wave is excited with such a strong amplitude, v0, that v0 vph, out of the applicability of the approximation. the breaking occurs also in inhomogeneous plasma, where due to the phase mixing effect 
...
By combining Eqs.~(\ref{eq:conservation}) and (\ref{eq:electron_and_photon_momentum_and_energy}), we obtain the following formula [\ref{paper_3}],
