...
\end{equation}
where $ \rho \left( \vec{r}, t \right) $ and $ \vec{J} \left( \vec{r}, t \right) $ are, respectively, the electric charge and current densities, and $ \varepsilon_0 $ and $ \mu_0 $ are the electric permittivity and the magnetic permeability of vacuum, respectively. By definition, $ \mu_0 \varepsilon_0 = 1 / c^2 $, where $ c $ is the velocity of light in vacuum.

By combining Eqs.~(\ref{eq:faraday_law}) and (\ref{eq:ampere_law}) and using the identity $ \rot{\left(\rot{\vec{E}}\right)} = \grad{\left(\div{\vec{E}}\right)} - \laplace{\vec{E}} $ we obtain the wave equation for $ \vec{E} $,
...
The motion of a single electron in the presence of $ \vec{E} $ and $ \vec{B} $ fields can be studied with the Lorentz equation,
