...
\end{equation}
where the last term is obtained using the identity $ \gamma_M + \sqrt{\gamma_M^2 - 1} = ( \gamma_M - \sqrt{\gamma_M^2 - 1} )^{-1} $ and $ \sqrt{\gamma_M^2 - 1} \approx \gamma_M $ and is valid in the relativistic limit, i.e., when $ \gamma_M \gg 1 $.
...
The spectral bandwidth of the outgoing wave packet in the stationary frame of reference is determined by $ \Delta \omega^{\prime 2} \left( T - R / c \right)^2 / 4 = \left[ \left( 1 + \beta_M \right) / \left( 1 - \beta_M \cos \theta \right) \right]^2 \Delta \omega^2 \left( t - \rho / c \right)^2 / 4 $ in Eqs.~(\ref{eq:envelope_function_1}) or (\ref{eq:envelope_function_2}), thus the duration is given by
\begin{equation}\label{key}
...
\end{equation}
where $ \omega_{pe} \left( \zeta \right) = \sqrt{e^2 n\left( \zeta \right) / \left( \varepsilon_0 m_e \gamma_e \right)} $ is the relativistic Langmuir frequency, $ \gamma_e $ is the electron Lorentz factor (note that $ \gamma_e = \gamma_M = 1 / \sqrt{1 - \beta_M^2} $ at the wave breaking coordinate $ \zeta_{br} $), $ n \left( \zeta \right) $ is the distribution of the electron density within the Langmuir wave, and $ \zeta = z - \beta_M c t $. 
...
\end{equation}
By plugging written in the compact form
\begin{equation}\label{key}
\diff*
{\begin{bmatrix}
	b_{+} \\
...
Assuming that the amplitude of the reflected wave is much smaller than the amplitude of the incident wave (i.e., $ \rho \ll 1 $) we may find that
\begin{equation}\label{key}
	\rho = \frac{i}{2 s} \int_{-\infty}^{+\infty} \nu \exp \left( -2 i s z^{\prime} \right) \dl z^{\prime}
\end{equation}

We consider the Langmuir wave at the threshold of wave breaking for which the electron density distribution is given by Eq.~\ref{eq:eq_for_density_breaking} 

%We assume $ a $ to be in the following form,
%\begin{equation}\label{key}
%a = a_0 \left( z^{\prime} \right) \exp \left[ i \left( k_y^{\prime} y^{\prime} - \omega_0^{\prime} t^{\prime} \right) \right].
%\end{equation}


It is the steepness of the cusps that affords the efficient reflection of a portion of the counterpropagating laser pulse. If the wake wave is far below the wave-breaking threshold, the reflection is exponentially small.

%In a
%strongly nonlinear wake wave, the electron density is modulated
%n such a way that the electrons form relatively thin
%layers moving with the velocity vph. A counterpropagating
%laser pulse called “source” interacts with the plasma, perturbed
%by the driver. Under certain conditions, the source is
%partially reflected from the wake wave, which thereby plays
%the role of a relativistic mirror. As the wake wave amplitude
%approaches the threshold for wave breaking i.e., when the
%electron velocity in the wave approaches its wave phase velocity
%, the electric field profile in the wave steepens and,
%correspondingly, the singularity is formed in the electron
%density profile. It is important that the singularity affords
%partial reflection of the signal with some reflection coefficient
%R

we consider
the interaction of an electromagnetic wave, representing the
signal, with a singularity of the electron density formed in a
breaking Langmuir wave.



Let us recall this scheme: an intese 
...
The calculations presented in Secs.~\ref{sec:rffm} and \ref{sec:rfpm} are valid only if the amplitude of the electromagnetic wave incident on the relativistic mirror is sufficiently low. When the amplitude is high, the electromagnetic wave may significantly affect the mirror motion (i.e., the radiation pressure of the wave may slow down or even destroy the mirror) even if the mirror reflection coefficient is small. The knowledge of the onset of recoil effects associated with the interaction of the relativistic mirror and strong incident electromagnetic wave is thus crucial for maximizing the amplitude of the reflected wave.
