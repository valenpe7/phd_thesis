...
\end{equation}
where $ \omega_{pe} \left( \zeta \right) = \sqrt{e^2 n\left( \zeta \right) / \left( \varepsilon_0 m_e \gamma_e \right)} $ is the relativistic Langmuir frequency, $ \gamma_e $ is the electron Lorentz factor (note that $ \gamma_e = \gamma_M = 1 / \sqrt{1 - v_M^2 / c^2} $ at the wave breaking coordinate $ \zeta_{br} $), $ n \left( \zeta \right) $ is the electron density within the Langmuir wave, and $ \zeta = z - v_M t $. We assume the Langmuir wave at the wave-breaking threshold; the distribution of the electron density can be thus generally described by
\begin{equation}\label{key}
...
\diffp[2]{a}{\zeta} + \left[ s^2 - \nu \right] a = 0.
\end{equation}
Here $ a \left( y^{\prime}, z^{\prime}; t^{\prime} \right) $ is the vector potential normalized by $ m_e c / e $, $ s^2 = \left( \omega_0^{\prime} / c \right)^2 - k_y^{\prime 2} $, and $ \nu \left( z^{\prime} \right) = \omega_{pe}^2\left( z^{\prime} \right) / c^2 $. We assume $ a $ to be in the following form,
\begin{equation}\label{key}
a = a_0 \left( z^{\prime} \right) \exp \left[ i \left( k_y^{\prime} y^{\prime} - \omega^{\prime} t^{\prime} \right) \right].
\end{equation}
We look for the solution of Eq.~(\ref{eq:refl_coeff_vector_potential_2}) 
